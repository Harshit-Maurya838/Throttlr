...
**Phase 3 Implementation Note**: As of Phase 3, this fail-open behavior is fully implemented. The logic resides in [getOrCreateBucket](file:///media/harshit-on-linux/ProgramFiles/Workspace/Projects/BackEnd_Projects/rate-limiter-service/src/services/bucketStore.ts#L66-L144) inside [bucketStore.ts](file:///media/harshit-on-linux/ProgramFiles/Workspace/Projects/BackEnd_Projects/rate-limiter-service/src/services/bucketStore.ts). When the Redis connection is closed (`!redis.isOpen` or `!redis.isReady`) or if reading state/writing state throws an error, the system logs an alert and defaults to a transient in-memory `TokenBucket` check, returning `degraded: true` and adding the `X-RateLimiter-Bypassed: true` header to bypass enforcement.
